...
## Statistical tests that can be used when your predictor is factorial
...
<sub> In other words, use it when your response and predictor are categorical <sub>
---

Example: Are flying pokemons are more legendary than expected by chance?
...
##### Now lets use the residuals to plot the chi-square data and findout how type is associated with color
...
res <- as.data.frame(a$residuals)
ggballoonplot(res, size = "Freq",fill = "Freq")+
...
Question: Does pokemon becomes legendery with the in increase in `speed`, `attack`, and `catch_rate` abilities?
---
